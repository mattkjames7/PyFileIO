...
class RenamedUnpickler(pickle._Unpickler):
	...
	def __init__(self,file, *, fix_imports=True, encoding='ASCII', errors='strict',OldNames=[],NewNames=[]):
	...
	def find_class(self, module, name):
		...
		print(renamed_module,name)
		return super(RenamedUnpickler, self).find_class(renamed_module, name)

	

def RenamedLoad(file_obj,fix_imports=True, encoding='ASCII', errors='strict',OldNames=[],NewNames=[]):
	'''
	I do not take credit for this function, if it works then thank the 
	person who wrote this post!
	https://stackoverflow.com/a/53327348
	...
	return RenamedUnpickler(file_obj,fix_imports=fix_imports,encoding=encoding,errors=errors,OldNames=OldNames,NewNames=NewNames).load()


def RenamedLoads(pickled_bytes,fix_imports=True, encoding='ASCII', errors='strict',OldNames=[],NewNames=[]):
	...
	return RenamedLoad(file_obj,fix_imports=fix_imports,encoding=encoding,errors=errors,OldNames=OldNames,NewNames=NewNames)
